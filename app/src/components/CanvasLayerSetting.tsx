...
import { layerSettingRegistryHashMap } from '@components/layer-canvas-registry';
  ...
  return (
    <div>
      {selectedLayerInfos.map(layerInfo => (
        <React.Fragment>
          <div style={{ display: 'block', textAlign: 'left', padding: '0px 20px 0px 30px' }}>
            <div style={{ ...MIDDLE_STYLE, justifyContent: 'space-between' }}>
              <h3>{layerInfo.name}</h3>
              <Space>
                {layerInfo.deletable && <CloseLayerButton layerId={layerInfo.id} />}
                {layerInfo.hideable && <HideLayerButton layerId={layerInfo.id} />}
                {layerInfo.hideable && <DuplicateButton layerId={layerInfo.id} />}
              </Space>
            </div>
            <Space>
              <Tag
                color={layerInfo.resizable ? 'blue' : GLOBAL_THEME_COLOR.$dark_text_color}
                style={{ margin: '0px 2px', borderRadius: 30 }}>
                Resizable
              </Tag>
              <Tag
                color={layerInfo.draggable ? 'blue' : GLOBAL_THEME_COLOR.$dark_text_color}
                style={{ margin: '0px 2px', borderRadius: 30 }}>
                Draggable
              </Tag>
              {hiddenLayers[layerInfo.id] && (
                <Tag color={'red'} style={{ margin: '0px 2px', borderRadius: 30 }}>
                  Hidden
                </Tag>
              )}
            </Space>
                ...
                children: (
                  <React.Fragment>
                    {layerInfo.settingSet[tab].map((component, index) => (
                      <React.Fragment>
                        {index > 0 && <Divider />}
                        {React.createElement(layerSettingRegistryHashMap[component], {
                          layer: layerInfo,
                        })}
                      </React.Fragment>
                    ))}
