...
const LayerListItem = ({ layer }: Props) => {
  ...
  const selected = useMemo(() => selectedLayers[layer.id], [selectedLayers, layer]);
  const hidden = useMemo(() => hiddenLayers[layer.id], [hiddenLayers, layer]);
  return (
    <div
      onClick={() => selectLayer(layer.id)}
      className={`canvas-layer-item ${selectedLayers[layer.id] ? 'selected' : ''}`}
      style={{ ...MIDDLE_STYLE, justifyContent: 'space-between', margin: '10px 0px' }}>
      <Tooltip title={layer.name}>
        <div style={{ ...MIDDLE_STYLE, justifyContent: 'flex-start', marginRight: 20 }}>
