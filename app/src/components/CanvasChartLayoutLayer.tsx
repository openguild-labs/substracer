...
const CanvasChartLayoutLayer: CanvasLayerComponent = ({ layer, children, ...props }) => {
  const { preview } = useCanvasStore();
  const { getChildLayerId } = useCanvasUtility();

  return (
    <CanvasLayer layer={layer} {...props}>
      <div style={{ padding: '10px 0px', ...MIDDLE_STYLE, flexDirection: 'column' }}>
        {preview ? (
          <React.Fragment>
            <RenderCanvasChildLayer parentLayer={layer} componentName="title" />
            <RenderCanvasChildLayer parentLayer={layer} componentName="subtitle" />
          </React.Fragment>
        ) : (
          <React.Fragment>
